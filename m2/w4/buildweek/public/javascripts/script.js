...
function avanti() {
  ...
  for (
    let l = currentPage * selectPages.value;
    l < selectPages.value * (currentPage + 1);
    l++
  ) {
...
function indietro() {
  ...
  for (
    let l = currentPage * selectPages.value;
    l < (currentPage + 1) * selectPages.value;
    l++
  ) {
    tabella.append(righe[l]);
  }
  ...
  .then((data) => {
    users = data.map((user) => {
      ...
      card.addEventListener("click", () => {
        ...
        let utente = array.find(
          (element) => element.querySelector(".fw-bold").innerHTML == user.id
        );
